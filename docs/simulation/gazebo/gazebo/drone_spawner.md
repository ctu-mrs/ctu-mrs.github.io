...
**NOTE**: The rosservice CLI may not allow the string to start with a dash. Add a space at the beginning to prevent errors.

Spawning multiple drones with the same configuration (useful for swarms). This example will spawn 5 drones, set the vehicle type to f450, add a Garmin rangefinder, blinking UV LEDs below the propellers, and 2 wide-angle cameras sensitive to UV light.
```bash
...
## Jinja syntax
Jinja is a templating language that allows the use of Python-esque syntax inside xml files. After rendering a template into an SDF model, all Jinja syntax is removed.
